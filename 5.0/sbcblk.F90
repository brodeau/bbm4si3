...
CONTAINS
   ...
   SUBROUTINE sbc_blk( kt )
      ...
      IF( MOD( kt - 1, nn_fsbc ) == 0 ) THEN
         ...
         ELSE
            ...
            zpre(:,:)         = pres_temp( q_air_zt(:,:), sf(jp_slp)%fnow(:,:,1), rn_zqt, pta=sf(jp_tair)%fnow(:,:,1) ) !LOLO: GS bug rn_zu => rn_zqt !#bbm
   ...
   SUBROUTINE blk_ice_1( pwndi, pwndj, ptair, pqair, pslp, ptsui,     &   ! inputs
      ...
      REAL(wp) , INTENT(  out), DIMENSION(A2D(0)  ), OPTIONAL ::   putaui_v  !#bbm
      REAL(wp) , INTENT(  out), DIMENSION(A2D(0)  ), OPTIONAL ::   pvtaui_u  !#bbm
      !
      INTEGER  ::   ji, jj    ! dummy loop indices
