...
# endif
   ...
   INTEGER , PARAMETER ::   jp_dmg = 11          ! index of ice damage       (-) !#bbm
...
CONTAINS
   ...
   SUBROUTINE ice_istate( kt, Kbb, Kmm, Kaa )
      ...
      REAL(wp), DIMENSION(jpi,jpj)     ::   zapnd_ini, zhpnd_ini, zhlid_ini, zdmgt_ini  !data from namelist or nc file
         ...
         IF ( ( Agrif_Root() ).OR.(.NOT.ln_init_chfrpar ) ) THEN
            ...
            IF( nn_iceini_file == 1 )THEN ! Read a file   !
               ...
               IF( TRIM(si(jp_dmg)%clrootname) == 'NOT USED' ) &
                  &     si(jp_dmg)%fnow(:,:,1) = ( rn_dmg_ini_n * zswitch + rn_dmg_ini_s * (1._wp - zswitch) ) * tmask(:,:,1)
               !
               ...
               zdmgt_ini(:,:) = si(jp_dmg)%fnow(:,:,1) * tmask(:,:,1) !#bbm
               !
            ...
            ELSE                          ! Read namelist !
               ...
               WHERE( ff_t(:,:) >= 0._wp )
                  ...
                  zdmgt_ini(:,:) = rn_dmg_ini_n * zswitch(:,:)
               ELSEWHERE
                  ...
                  zdmgt_ini(:,:) = rn_dmg_ini_s * zswitch(:,:)
               END WHERE
               !
            ENDIF
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namini in configuration namelist' )
      ...
      IF(lwp) THEN                          ! control print
         ...
         IF( ln_iceini .AND. nn_iceini_file == 0 ) THEN
            ...
            WRITE(numout,*) '      initial pnd lid depth in the north-south         rn_dmg_ini     = ', rn_dmg_ini_n,rn_dmg_ini_s
         ENDIF
      ENDIF
      !
      ...
      !
      IF( .NOT.ln_damage ) THEN
         rn_dmg_ini_n = 0. ; rn_dmg_ini_s = 0. !#bbm
      ENDIF
      !
